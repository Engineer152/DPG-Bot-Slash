...
slash = SlashCommand(client, sync_commands=True,override_type = True,sync_on_cog_reload=True,delete_from_unused_guilds=True)
...
class Slash(commands.Cog):
  ...
  async def _suggestsong(self, ctx, **name):
      ...
      embed.set_author(name = ctx.author, icon_url = ctx.author.avatar_url)
      await ctx.send(content = f"{ctx.author.mention} **Your song suggestion has been submitted. The admins will review and reach out to you if approved.**", hidden = True)
      suggest = await suggest_channel.send(content = None, embed = embed)
      await suggest.add_reaction("👍")
      await suggest.add_reaction("👎")
...
#inside the class
  ...
  async def vote(self,ctx,member:discord.Member=None):
    ...
    await ctx.send(f"https://engineer15.sirv.com/Images/Among%20Us/{random.choice(colors)}.png?text={name}%20{random.choice(bools)}&text.size=40&text.color=0D0D0D&text.position.gravity=center&text.position.x=0%&text.position.y=15%")
