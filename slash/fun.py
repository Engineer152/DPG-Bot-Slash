...
class fun(commands.Cog):
    ...
    async def vote(self,
        ...
        if interaction.channel.id != 758297067155488799:
            self.vote.reset_cooldown(interaction)
    ...
    @commands.slash_command(description="Call an emergency meeting",
                            guild_ids=[720657696407420950])
    async def button(self, interaction: discord.CommandInteraction, *, txt=None):
        if interaction.channel.id != 758297067155488799:
            self.vote.reset_cooldown(interaction)
            return await interaction.response.send_message(
                "You can't use this here try it in <#759526501598101506>",
        ...
        if txt is None:
            txt = "%20"
        name = txt.replace(" ", "%20")
        await interaction.send(f"https://vacefron.nl/api/emergencymeeting?text={name}")

    @vote.error
    async def vote_error(self, interaction: discord.CommandInteraction, error):
    ...
    @commands.slash_command(
        ...
        guild_ids=[720657696407420950])
    async def _bottlebust(self, interaction: discord.CommandInteraction, frenemy: discord.Member = commands.Param(description="Who will you bottle bust?",
                                                                                                                  default=False)):
        if interaction.channel.id != 758297067155488799:
            return await interaction.respose.send_message("Wrong channel\nPlease use <#758297067155488799>", ephemeral=True)
        if not frenemy:
            frenemy = interaction.author
        embed = discord.Embed(title=":rotating_light:             BOTTLE BUSTER ALERT            :rotating_light:",
                              description=f"{interaction.author.mention} **BOTTLE BUSTED** {frenemy.mention}")
    ...
    async def _dance(self, interaction: discord.CommandInteraction):
        embed = discord.Embed(title='PANDA DANCE PARTY!')
        embed.set_image(
            url='https://media.tenor.com/images/b0dcde6ad525effd15648871cf6810f6/tenor.gif')
        await interaction.response.send_message(embed=embed)

    @commands.slash_command(name="cf", description=" Heads or tails, let the DPG BOT decide!",
                            guild_ids=[720657696407420950])
    async def _cf(self, interaction: discord.CommandInteraction):
        if interaction.channel.id != 758297067155488799:
        ...
        if x == 1:
            ...
            await interaction.response.send_message(content=None, embeds=[embed])
            await asyncio.sleep(5)
    ...
    @commands.slash_command(description="Take your chance at an awesome DP trickshot. Can you do it?!",
                            guild_ids=[720657696407420950])
    async def trickshot(self, interaction: discord.CommandInteraction):
        if interaction.channel.id != 758297067155488799:
            return await interaction.response.send_message("Wrong channel\nPlease use <#758297067155488799>", ephemeral=True)
        ...
        if rng == 5:
            with open("gtrickshot.txt") as questions:
                ...
                for line in questions:
                    lines.append(line)
            embed = discord.Embed(
                title=f'{lines[random.randint(0, len(lines) - 1)]}', color=0x5fb71b)
            ...
            with open("btrickshot.txt") as questions:
                ...
                for line in questions:
                    lines.append(line)
            embed = discord.Embed(
                title=f'{lines[random.randint(0, len(lines) - 1)]}', color=0xe74c3c)
            await interaction.response.send_message(embed=embed)

    @commands.slash_command(name="signoff", description="Heading out? Hit us with the DP Sign-off!",
                            guild_ids=[720657696407420950])
    async def _signoff(self, interaction: discord.CommandInteraction):
        embed = discord.Embed(
            description=f'**{interaction.author.mention} is signing off for now...**', color=0x46e2ec)
        embed.add_field(name='POUND IT! :punch:', value='\u200b', inline=False)
        embed.add_field(name='NOGGIN! :busts_in_silhouette:',
                        value='\u200b', inline=False)
        embed.add_field(name='SEE YA!! :v:', value='\u200b', inline=False)
        embed.add_field(
            name=':billed_cap:     :arrow_right:     :movie_camera:', value='\u200b', inline=False)
        embed.set_image(
    ...
    async def dpwiki(self, interaction: discord.CommandInteraction):
        if interaction.channel.id != 758297067155488799:
            return await interaction.response.send_message("Wrong channel\nPlease use <#758297067155488799>", ephemeral=True)
...
#                            guild_ids=[720657696407420950])
#    async def _minecraftserver(self, interaction:discord.CommandInteraction):
...
#        await interaction.response.send_message(":mailbox: You've Got Mail!", ephemeral=True)
#        try:
#            message = await interaction.author.send(embed=embed)
...
#            content=f"[Click here to view your mail :mailbox:]({message.jump_url})")
    ...
    @commands.slash_command(description="RIP", name="rip",
                            guild_ids=[720657696407420950])
    async def _rip(self, interaction: discord.CommandInteraction):
