...
class info(commands.Cog):
    ...
    async def rules(self, interaction: discord.CommandInteraction):
        embed = discord.Embed(
        ...
        embed.set_thumbnail(
            url="https://i.ibb.co/bLJR1Zz/Know-the-Rules-320x242.png")
    ...
    async def youtube(self, interaction: discord.CommandInteraction):
    ...
    async def social(self, interaction: discord.CommandInteraction):
        if interaction.channel.id != 758297067155488799:
            return await interaction.send("Wrong channel\nPlease use <#758297067155488799>", ephemeral=True)
        embed = discord.Embed(
            title='Socials', description="Check out the Dude's socials", color=0x46e2ec)
    ...
    async def tiethepie(self, interaction: discord.CommandInteraction):
        if interaction.channel.id != 758297067155488799:
            return await interaction.response.send_message("Wrong channel\nPlease use <#758297067155488799>", ephemeral=True)
    ...
    async def dpbeans(self, interaction: discord.CommandInteraction):
        if interaction.channel.id != 758297067155488799:
            return await interaction.response.send_message("Wrong channel\nPlease use <#758297067155488799>", ephemeral=True)
        embed = discord.Embed(title="<:DPBeans:816310052595433512:DUDE PERFECT BEANS!!<:DPBeans:816310052595433512:",
                              ...
                              color=0x46e2ec)
        embed.set_thumbnail(
            url="https://i.ibb.co/JzRdVkr/Serious-Bean-Co-Jalapeno-Bacon-Article.png")
        await interaction.response.send_message(embed=embed)

    @commands.slash_command(description="The Overtime intro", guild_ids=[720657696407420950])
    async def intro(self, interaction: discord.CommandInteraction):
        if interaction.channel.id != 758297067155488799:
            return await interaction.response.send_message("Wrong channel\nPlease use <#758297067155488799>", ephemeral=True)
        embed = discord.Embed(
    ...
    async def playlist(self, interaction: discord.CommandInteraction):
        if interaction.channel.id != 758297067155488799:
            return await interaction.response.send_message("Wrong channel\nPlease use <#758297067155488799>", ephemeral=True)
